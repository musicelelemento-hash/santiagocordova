...
export const ClientDetailView: React.FC<ClientDetailViewProps> = memo(({ client, onSave, onBack, serviceFees, sriCredentials }) => {
    ...
    const { whatsappTemplates, setTasks, clients } = useAppStore();
    ...
    const handlePdfUpdate = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (!passwordToUse && sriCredentials && sriCredentials[extracted.ruc]) {
                ...
                toast.success("Clave sincronizada de Bóveda");
            }
            ...
            toast.success("Documento certificado SRI validado.");
        } catch (error) {
            toast.error("No se pudo validar el certificado PDF.");
        } finally {
            setIsAnalyzingPdf(false);
    ...
    const handleJumpToOwner = () => {
        ...
        if (owner) {
            ...
            toast.info("El cliente no existe. Créelo desde el menú de Clientes.");
            setMismatchData(null);
        }
    };
    ...
    const handleSave = () => {
        ...
        const finalFeeStructure = {
            monthly: parseFloat(monthlyFee) || 5,
            annual: parseFloat(annualFee) || 10,
            semestral: parseFloat(monthlyFee) || 5 
        };
...
Nota: Este enlace es personal y seguro.`;
    ...
    return (
        <div className="bg-slate-50 dark:bg-slate-950 min-h-screen flex flex-col animate-fade-in absolute inset-0 z-50 overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in-up">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-sm border border-slate-100 dark:border-slate-800">
                                    ...
                                    {isEditing && (
                                        <div className="relative overflow-hidden group">
                                            ...
                                            <button 
                                                ...
                                                className="flex items-center gap-2 px-6 py-2.5 bg-brand-teal/10 text-brand-teal border border-brand-teal/20 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-brand-teal hover:text-white transition-all shadow-sm"
                                            >
                                                {isAnalyzingPdf ? <Loader size={12} className="animate-spin"/> : <ScanLine size={14}/>}
                                                {isAnalyzingPdf ? 'Analizando...' : 'Validar con Certificado PDF'}
                                            </button>
                                        </div>
                                    )}
                                </div>
                                ...
                                {/* HONORARIOS / TARIFAS (NEW) */}
                                <div className="space-y-4 mb-6">
                                    <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-700 pb-2 mb-3 flex items-center gap-2">
                                        <Coins size={14} className="text-emerald-500"/> Honorarios Profesionales
                                    </h4>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            {isEditing ? (
                                                <div className="relative flex items-center">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-full pl-8 p-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl text-sm font-bold focus:ring-2 focus:ring-emerald-500 outline-none"
                                                    />
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-2">
                                                    <DollarSign size={16} className="text-emerald-500"/>
                                                    <span className="text-lg font-black text-slate-800 dark:text-white">{parseFloat(monthlyFee).toFixed(2)}</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="p-4 rounded-2xl bg-emerald-50/30 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30">
                                            ...
                                            {isEditing ? (
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <DollarSign size={16} className="text-emerald-500"/>
                                                    <span className="text-lg font-black text-slate-800 dark:text-white">{parseFloat(annualFee).toFixed(2)}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                        ...
                        <div className="bg-yellow-50 dark:bg-yellow-900/10 p-6 rounded-3xl border border-yellow-200 dark:border-yellow-800/50 shadow-sm h-full flex flex-col">
                            ...
                            {isEditing ? (
                                <textarea
                                    ...
                                    className="w-full flex-1 bg-white/50 p-4 rounded-xl text-sm border-none focus:ring-2 focus:ring-yellow-400 resize-none min-h-[200px]"
                                />
                            ) : (
                                <div className="prose prose-sm prose-yellow max-w-none text-yellow-900 dark:text-yellow-100/80 whitespace-pre-wrap flex-1">
                                    {editedClient.notes || 'No hay notas registradas.'}
                                </div>
                            )}
                        </div>
                    </div>
             ...
             {/* MODAL ERROR DE RUC */}
             <Modal isOpen={!!mismatchData} onClose={() => setMismatchData(null)} title="Inconsistencia de Identidad">
                 <div className="text-center p-4">
                     ...
                     <h3 className="text-xl font-black text-slate-800 mb-2 uppercase tracking-tight">¡RUC NO COINCIDE!</h3>
                     <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                         El certificado SRI pertenece a <strong className="text-slate-800">{mismatchData?.name}</strong> (RUC {mismatchData?.ruc}), 
                         pero usted está editando a <strong className="text-slate-800">{client.name}</strong>.
                         <br/><br/>
                         Por seguridad, no se han modificado los datos del cliente actual.
                     </p>
                     
                     <div className="flex flex-col gap-3">
                         <button 
                            onClick={handleJumpToOwner}
                            className="w-full py-4 bg-brand-navy text-white font-black rounded-2xl flex items-center justify-center gap-2 shadow-xl"
                         >
                             <ArrowRight size={18}/> Ver Ficha de {mismatchData?.name.split(' ')[0]}
                         </button>
                         <button 
             ...
             {/* Modales de Confirmación y Recibo */}
             {confirmation && (
                <Modal isOpen={!!confirmation} onClose={() => setConfirmation(null)} title="Confirmar Acción">
                    <div className="text-center p-4">
                        ...
                        <div className="flex flex-col gap-3">
                            <button onClick={() => handleConfirmAction()} disabled={isProcessingAction} className="w-full py-3.5 bg-brand-navy text-white font-bold rounded-xl shadow-lg hover:bg-slate-800 transition-all">
                                {isProcessingAction ? <Loader className="animate-spin mx-auto"/> : 'Solo Confirmar'}
                            </button>
                            {confirmation.action === 'declare' && (
                                <button onClick={() => handleConfirmAction(true)} disabled={isProcessingAction} className="w-full py-3.5 bg-green-600 text-white font-bold rounded-xl shadow-lg hover:bg-green-700 flex items-center justify-center gap-2">
                                    <MessageCircle size={18}/> Confirmar y Notificar
                                </button>
                            )}
                            <button onClick={() => setConfirmation(null)} className="w-full py-3 text-slate-500 hover:bg-slate-100 rounded-xl font-medium">Cancelar</button>
                        </div>
                    </div>
                </Modal>
             )}
