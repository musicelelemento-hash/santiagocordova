...
import { Task, TaskStatus, Client, Declaration, DeclarationStatus, TaxRegime, ServiceFeesConfig, TranscribableField } from './types';
import { Plus, Search, Eye, EyeOff, Copy, Edit, UserPlus, BrainCircuit, Loader, ArrowDownToLine, X, ChevronUp, ChevronDown, Sparkles } from 'lucide-react';
import { Modal } from './Modal';
import { v4 as uuidv4 } from 'uuid';
import { format, isPast, addDays } from 'date-fns';
import { es } from 'date-fns/locale/es';
import { getAnnualIncomeTaxDueDate, getAnexoDueDate, validateRuc, validateSriPassword, getPeriod, getIdentifierSortKey } from './sri';
import { getClientServiceFee } from './clientService';
import { analyzeClientPhoto } from './geminiService';
import { useTranscription } from './useTranscription';


type TaskType = 'generic' | 'annual_renta' | 'devolucion_iva' | 'devolucion_renta' | 'anexo_gastos';
...
const REQUIRES_IVA = (client: Client) => client.taxProfile?.ivaFrequency !== 'Ninguno';

const TaskDetailView: React.FC<{ task: Task; onSave: (updatedTask: Task) => void; onBack: () => void; clients: Client[] }> = ({ task, onSave, onBack, clients }) => {
                ...
                {isEditing ? (
                    <div className="grid grid-cols-2 gap-4 pt-2">
                        <div className="relative">
                            ...
                            <input type="number" placeholder="0.00" value={editedTask.cost || ''} onChange={e => setEditedTask({...editedTask, cost: parseFloat(e.target.value) || undefined})} className="w-full p-2 pl-6 mt-1 bg-gray-100 dark:bg-gray-800 rounded"/>
                        </div>
                        <div className="relative">
...
export const TasksScreen: React.FC<TasksScreenProps> = ({ tasks, setTasks, clients, setClients, taskFilter, clearTaskFilter, serviceFees, initialTaskData, clearInitialTaskData }) => {
  ...
  const [rucSuggestion, setRucSuggestion] = useState('');
