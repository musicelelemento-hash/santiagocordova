...
export const TasksScreen: React.FC<TasksScreenProps> = ({ navigate, taskFilter, clearTaskFilter, initialTaskData, clearInitialTaskData }) => {
    ...
    const filteredTasks = useMemo(() => {
        ...
        clients.filter(c => !c.isDeleted && (!taskFilter?.clientId || c.id === taskFilter.clientId)).forEach(client => {
            client.declarationHistory.forEach(decl => {
                if (decl.isPaid && !decl.proofFile) {
                    const dueDate = getDueDateForPeriod(client, decl.period);
                    autoTasks.push({
                        id: `auto-iva-${client.id}-${decl.period}`,
                        title: `ORDEN: Declaración IVA ${formatPeriodForDisplay(decl.period)}`,
            ...
            const checkSpecial = (idSuffix: string, titleLabel: string, isPaid: boolean, hasProof: boolean, period: string) => {
                if (isPaid && !hasProof) {
                    const dueDate = getDueDateForPeriod(client, period);
                    autoTasks.push({
                        id: `auto-${idSuffix}-${client.id}-${period}`,
                        title: `ORDEN: ${titleLabel} ${period}`,
                        description: `⚠️ Honorarios de ${titleLabel} pagados. Pendiente liquidar gestión.`,
                        dueDate: dueDate?.toISOString() || today.toISOString(),
                        status: TaskStatus.Pendiente,
                        clientId: client.id,
                        isAutoGenerated: true
            ...
            checkSpecial('renta', 'Declaración Renta', !!client.annualRentaPaid, !!client.annualRentaProof, prevYearStr);
            checkSpecial('devolucion', 'Devolución IVA', !!client.devolucionIvaPaid, !!client.devolucionIvaProof, currentYearStr);
            checkSpecial('ice-anexo', 'Anexo ICE', !!client.iceAnexoPaid, !!client.iceAnexoProof, currentYearStr);
            checkSpecial('ice-decl', 'Declaración ICE', !!client.iceDeclarationPaid, !!client.iceDeclarationProof, currentYearStr);
            checkSpecial('pvp', 'Anexo PVP', !!client.anexoPvpPaid, !!client.anexoPvpProof, currentYearStr);
        });
    ...
    const stats = useMemo(() => {
        ...
        const autoCount = clients.reduce((acc, client) => {
            ...
            const ivaPendingCount = client.declarationHistory.filter(d => d.isPaid && !d.proofFile).length;
            const isRentaPending = !!client.annualRentaPaid && !client.annualRentaProof;
            const isDevolucionPending = !!client.devolucionIvaPaid && !client.devolucionIvaProof;
            const isIceAnexoPending = !!client.iceAnexoPaid && !client.iceAnexoProof;
            const isIceDeclPending = !!client.iceDeclarationPaid && !client.iceDeclarationProof;
            const isPvpPending = !!client.anexoPvpPaid && !client.anexoPvpProof;
            const specialsCount = [isRentaPending, isDevolucionPending, isIceAnexoPending, isIceDeclPending, isPvpPending].filter(Boolean).length;
            return acc + ivaPendingCount + specialsCount;
        }, 0);
