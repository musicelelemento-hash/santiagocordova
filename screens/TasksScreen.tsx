...
export const TasksScreen: React.FC<TasksScreenProps> = ({ navigate, taskFilter, clearTaskFilter, initialTaskData, clearInitialTaskData }) => {
    ...
    const [activeTab, setActiveTab] = useState<'all' | 'orders' | 'pending' | 'completed'>('all');
    ...
    const filteredTasks = useMemo(() => {
        ...
        if (activeTab === 'orders') {
            tabFiltered = combined.filter(t => (t as any).isAutoGenerated);
        } else if (activeTab === 'pending') {
            tabFiltered = combined.filter(t => (t as any).isAutoGenerated || (t.status !== TaskStatus.Completada && t.status !== TaskStatus.Pagada));
    ...
    const stats = useMemo(() => {
        ...
        const pendingManual = tasks.filter(t => t.status !== TaskStatus.Completada && t.status !== TaskStatus.Pagada).length;
        const completedManual = tasks.filter(t => t.status === TaskStatus.Completada || t.status === TaskStatus.Pagada).length;

        return { 
            total: tasks.length + autoCount, 
            orders: autoCount, 
            urgent: urgentCount,
            pending: pendingManual + autoCount,
            completed: completedManual
        };
    }, [tasks, clients]);
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto font-mono">
                            <div className="flex p-1.5 bg-[#0b1326] rounded-2xl border border-white/10 overflow-x-auto no-scrollbar">
                                ...
                                >
                                    Work Orders
                                    {stats.orders > 0 && (
                                        <span className="ml-1.5 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-[#C9A96E] text-slate-950 font-mono">
                                            {stats.orders}
                                        </span>
                                    )}
                                </button>
                                ...
                                >
                                    Pendientes
                                    {stats.pending > 0 && (
                                        <span className="ml-1.5 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-amber-500/20 text-amber-300 font-mono">
                                            {stats.pending}
                                        </span>
                                    )}
                                </button>
