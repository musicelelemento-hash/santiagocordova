...
const WhatsAppWidget = ({ phoneNumber }: { phoneNumber: string }) => {
    return (
        <div className="fixed bottom-6 right-6 z-[100] no-print">
            <a href={`https://wa.me/${phoneNumber}?text=Hola%20Ing.%20Santiago,%20quisiera%20una%20consulta.`} target="_blank" rel="noopener noreferrer" className="relative flex items-center justify-center w-16 h-16 bg-[#25D366] rounded-full text-white shadow-2xl hover:scale-110 active:scale-95 transition-all duration-300 group">
...
const NewsSection = () => {
    const news = [
        { title: "Nuevos Límites RIMPE 2026", date: "Actualización SRI", icon: LucideIcons.TrendingUp },
        { title: "Cambios en Retenciones de IVA", date: "Alerta Fiscal", icon: LucideIcons.AlertTriangle },
        { title: "Obligaciones Marzo 2026", date: "Calendario", icon: LucideIcons.CalendarClock }
    ];
    return (
        <section id="noticias" className="py-24 relative overflow-hidden border-t border-white/5">
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                <div className="mb-12 flex items-center justify-between">
                    <div>
                        <div className="text-[10px] font-bold text-[#00A896] uppercase tracking-[0.4em] mb-2">— Actualidad Fiscal</div>
                        <h2 className="text-3xl md:text-5xl font-editorial tracking-tighter text-white">NOTICIAS TRIBUTARIAS</h2>
                    </div>
                    <button className="hidden md:flex items-center gap-2 text-sm text-[#00A896] hover:text-white transition-colors">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {news.map((item, i) => (
                        <Reveal key={i} delay={i * 100}>
                            <div className="p-8 rounded-3xl bg-white/5 border border-white/10 hover:border-[#00A896]/40 transition-all duration-300 group cursor-pointer glass-premium-2 h-full flex flex-col">
                                ...
                                <h3 className="text-xl font-bold text-white mb-4 group-hover:text-[#00A896] transition-colors">{item.title}</h3>
                                <div className="flex items-center gap-2 text-sm text-slate-400 group-hover:text-white transition-colors mt-auto pt-4 border-t border-white/5">
...
// ─── LEAD MAGNET BANNER ───────────────────────────────────────────────────────
const LeadMagnetBanner = ({ phoneNumber }: { phoneNumber: string }) => {
    return (
        <section className="py-20 relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-r from-[#00A896]/10 to-blue-900/10" />
            <div className="max-w-5xl mx-auto px-6 relative z-10">
                <div className="p-10 md:p-14 rounded-[3rem] border border-[#00A896]/30 bg-[#020617]/80 backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-10 shadow-[0_0_50px_rgba(0,168,150,0.15)] overflow-hidden relative">
                    ...
                    <div className="flex-1 text-center md:text-left relative z-10">
                        ...
                        <h2 className="text-3xl md:text-5xl font-editorial tracking-tighter text-white mb-4">CALENDARIO TRIBUTARIO 2026</h2>
                        <p className="text-slate-400 text-lg font-light mb-0 max-w-xl">Descarga la guía en PDF con todas las fechas de vencimiento de SRI, IESS y Supercias para este año.</p>
                    </div>
                    <div className="flex-shrink-0 relative z-10">
                        <a href={`https://wa.me/${phoneNumber}?text=Hola,%20quisiera%20descargar%20el%20Calendario%20Tributario%202026.`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-[#00A896] text-white rounded-full font-bold uppercase tracking-widest hover:bg-white hover:text-[#00A896] transition-all duration-300 shadow-xl hover:shadow-[0_0_30px_rgba(0,168,150,0.4)]">
...
export const LandingPage: React.FC<LandingPageProps> = ({ onAdminAccess, onNavigateToServices, theme = 'dark', toggleTheme }) => {
    ...
    return (
        <div className={`${theme === 'dark' ? 'bg-[#020617] text-slate-200' : 'bg-slate-50 text-slate-800'} min-h-screen selection:bg-[#00A896]/30 selection:text-white font-sans overflow-x-hidden transition-colors duration-500`}>
            ...
            <div className="fixed top-0 left-0 right-0 z-50 flex flex-col items-center pt-5 px-4 pointer-events-none">
                ...
                <nav className={`pointer-events-auto transition-all duration-700 flex items-center justify-between px-3 pr-3 py-2 rounded-full border shadow-2xl backdrop-blur-3xl
                    ...
                    <div className="flex items-center gap-2">
                        {toggleTheme && (
                            <button onClick={toggleTheme} className="hidden md:flex items-center justify-center w-8 h-8 rounded-full border border-white/20 bg-black/20 text-slate-300 hover:text-white transition-all mr-2 hover:bg-white/10">
            ...
            {/* ══════════════════════════════════════════════════════════════
                NOTICIAS Y LEAD MAGNET
            ══════════════════════════════════════════════════════════════ */}
            <NewsSection />
            <LeadMagnetBanner phoneNumber={phoneNumber} />
