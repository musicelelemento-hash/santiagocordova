...
export const ThreeDStudioScreen: React.FC = () => {
    ...
    return (
        <div 
            ref={containerRef}
            className="relative w-full h-[calc(100vh-4rem)] overflow-y-auto bg-surface text-on-surface scroll-smooth selection:bg-primary selection:text-white"
        >
            ...
            <div className="relative z-10 -mt-[100vh]">
                ...
                <div className="sticky top-4 left-4 right-4 mx-auto max-w-6xl z-40 bg-surface-low/80 backdrop-blur-xl border border-primary/20 rounded-2xl p-4 shadow-2xl flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-bold bg-gradient-to-r from-white via-primary/40 to-primary bg-clip-text text-transparent">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="hidden sm:flex items-center gap-2 bg-surface border border-foreground/10 px-3 py-1.5 rounded-xl text-xs text-primary font-mono">
                            <span>Scroll Progress:</span>
                            <span className="font-bold text-on-surface">{Math.round(scrollProgress * 100)}%</span>
                        </div>

                        <button
                            onClick={() => setIsMcpModalOpen(true)}
                            className="py-2 px-4 bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 text-white font-medium text-xs rounded-xl shadow-lg shadow-primary/20 flex items-center gap-2 transition-all hover:scale-105"
                        >
                ...
                <section className="min-h-screen flex flex-col justify-center items-center px-6 text-center max-w-4xl mx-auto">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary/10 border border-primary/30 text-primary text-xs font-semibold uppercase tracking-widest mb-6 animate-pulse">
                        <Zap className="w-3.5 h-3.5" /> Next-Gen 3D Interactive Design
                    </div>
                    <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight bg-gradient-to-r from-white via-slate-100 to-primary bg-clip-text text-transparent mb-6">
                        Experiencias 3D Guiadas por Scroll
                    </h1>
                    <p className="text-lg md:text-xl text-on-surface-variant max-w-2xl font-light mb-10 leading-relaxed">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                        <button
                            onClick={() => setIsMcpModalOpen(true)}
                            className="w-full sm:w-auto py-3.5 px-8 bg-primary hover:bg-primary/90 text-white font-semibold rounded-xl shadow-xl shadow-primary/30 flex items-center justify-center gap-2 transition-all hover:scale-105"
                        >
                            <Sparkles className="w-5 h-5" /> Generar con Alpha3D AI
                        </button>
                        <div className="flex items-center gap-2 text-xs text-on-surface-variant">
                            <ChevronDown className="w-4 h-4 animate-bounce text-primary" /> Desliza hacia abajo para transformar el objeto 3D
                        </div>
                    </div>
                </section>
                ...
                <section className="min-h-screen flex items-center justify-start px-6 md:px-16 max-w-6xl mx-auto">
                    <div className="bg-surface-low/70 border border-foreground/10 backdrop-blur-xl p-8 md:p-10 rounded-3xl max-w-lg shadow-2xl space-y-4">
                ...
                <section className="min-h-screen flex items-center justify-end px-6 md:px-16 max-w-6xl mx-auto">
                    <div className="bg-surface-low/70 border border-foreground/10 backdrop-blur-xl p-8 md:p-10 rounded-3xl max-w-lg shadow-2xl space-y-4 text-right">
                ...
                <section className="min-h-screen flex flex-col justify-center items-center px-6 text-center max-w-5xl mx-auto">
                    <div className="bg-gradient-to-b from-primary/15 to-surface-low/90 border border-primary/30 backdrop-blur-2xl p-10 md:p-14 rounded-3xl shadow-2xl space-y-6 max-w-3xl">
                        <div className="p-4 bg-primary/30 border border-primary/40 rounded-2xl w-fit mx-auto text-primary">
                        ...
                        <div className="pt-4 flex flex-col sm:flex-row justify-center items-center gap-4">
                            <button
                                onClick={() => setIsMcpModalOpen(true)}
                                className="w-full sm:w-auto py-3.5 px-8 bg-gradient-to-r from-primary to-secondary hover:from-primary/90 hover:to-secondary/90 text-white font-semibold rounded-xl shadow-xl shadow-primary/30 flex items-center justify-center gap-2 transition-all hover:scale-105"
                            >
