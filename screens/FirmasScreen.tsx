...
export const FirmasScreen: React.FC<FirmasScreenProps> = ({ navigate }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-300 pb-24 font-sans">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] border border-foreground/10 border-t-foreground/20 bg-surface-low shadow-2xl backdrop-blur-2xl p-8 md:p-10">
                <div className="absolute top-0 right-0 w-96 h-96 bg-tertiary/10 rounded-full blur-3xl -mr-10 -mt-10 pointer-events-none" />
                <div className="absolute bottom-0 left-0 w-96 h-96 bg-primary/10 rounded-full blur-3xl -ml-10 -mb-10 pointer-events-none" />

                <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 rounded-3xl bg-tertiary/15 border border-tertiary/30 text-tertiary shadow-[0_0_15px_rgba(0,168,150,0.3)] shrink-0">
                            <KeyRound size={32} strokeWidth={2.2} />
                        </div>
                        <div>
                            <div className="flex items-center gap-2 mb-1.5">
                                <div className="w-2 h-2 rounded-full bg-tertiary animate-pulse shadow-[0_0_8px_rgba(0,168,150,0.8)]" />
                                <span className="text-[9px] font-bold text-tertiary uppercase tracking-[0.3em]">Auditoría de Certificados Digitales SRI</span>
                            </div>
                            ...
                            <p className="text-xs sm:text-sm text-on-surface-variant mt-1 font-medium font-sans">
                    ...
                    <div className="flex items-center gap-3 flex-wrap shrink-0">
                        <div className="flex flex-col items-center px-4 py-3 rounded-2xl bg-tertiary/15 border border-tertiary/30 shadow-sm">
                            <span className="text-2xl font-black text-tertiary font-mono">{signatureData.ok.length}</span>
                            <span className="text-[8.5px] font-bold text-tertiary uppercase tracking-wider mt-0.5">Válidas / Activas</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center px-4 py-3 rounded-2xl bg-primary/15 border border-primary/30 shadow-sm">
                            <div className="flex items-center gap-1 text-primary font-black text-xl">
                                <Cloud size={16} />
                                <span>Cloud R2</span>
                            </div>
                            <span className="text-[8.5px] font-bold text-primary uppercase tracking-wider mt-0.5">Cloudflare Synced</span>
                        </div>

                        {/* Botón de Subida Masiva */}
                        <button
                            onClick={() => setIsBulkModalOpen(true)}
                            className="px-5 py-3 rounded-2xl bg-gradient-to-r from-tertiary to-tertiary hover:from-tertiary hover:to-tertiary text-white font-bold text-xs uppercase tracking-wider transition-all shadow-lg shadow-tertiary/25 active:scale-95 flex items-center gap-2 border border-foreground/10 cursor-pointer"
                        >
                            <UploadCloud size={16} />
            ...
            <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
                <div className="flex flex-wrap items-center gap-3">
                    {/* Pestañas de Estado */}
                    <div className="flex items-center gap-1.5 bg-surface-lowest p-1.5 rounded-2xl border border-foreground/10 w-fit">
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                                tab === 'vigentes'
                                    ? 'bg-foreground/15 text-on-surface shadow-lg border border-foreground/20'
                                    : 'text-on-surface-variant hover:text-on-surface'
                            }`}
                        >
                            <ShieldCheck size={14} className={tab === 'vigentes' ? 'text-tertiary' : ''} />
                            <span>Clientes Activos</span>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${tab === 'vigentes' ? 'bg-tertiary/20 text-tertiary border border-tertiary/30' : 'bg-foreground/10 text-on-surface-variant'}`}>
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all duration-300 cursor-pointer ${
                                tab === 'con-ruc'
                                    ? 'bg-tertiary/20 text-tertiary shadow-lg border border-tertiary/30'
                                    : 'text-on-surface-variant hover:text-on-surface'
                            }`}
                        >
                            <Laptop size={14} className={tab === 'con-ruc' ? 'text-tertiary' : ''} />
                            <span>Con RUC (Para Facturar)</span>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${tab === 'con-ruc' ? 'bg-tertiary text-white' : 'bg-foreground/10 text-on-surface-variant'}`}>
                        ...
                        >
                            ...
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${tab === 'solo-firma' ? 'bg-amber-500 text-white' : 'bg-foreground/10 text-on-surface-variant'}`}>
                    ...
                    {(tab !== 'respaldos-externos') && (
                        <div className="flex items-center gap-1 bg-surface-lowest p-1 rounded-2xl border border-foreground/10">
                            <button
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none pl-8 pr-8 py-2 bg-surface-lowest border border-foreground/10 rounded-2xl text-xs text-on-surface focus:border-tertiary/50 transition-all outline-none cursor-pointer font-mono"
                        >
                            <option value="all" className="bg-surface-low text-on-surface">Todos los Emisores</option>
                            <option value="Security Data" className="bg-surface-low text-on-surface">Security Data</option>
                            <option value="ANF" className="bg-surface-low text-on-surface">ANF AC</option>
                            <option value="Banco Central" className="bg-surface-low text-on-surface">Banco Central (BCE)</option>
                            <option value="UANATACA" className="bg-surface-low text-on-surface">UANATACA</option>
                            <option value="Consejo de la Judicatura" className="bg-surface-low text-on-surface">Consejo Judicatura</option>
                        </select>
                        <Filter size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-on-surface-variant pointer-events-none" />
                    </div>
                    ...
                    <div className="relative min-w-[220px] sm:w-72">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2 bg-surface-lowest border border-foreground/10 rounded-2xl text-xs text-on-surface placeholder-on-surface-variant focus:border-tertiary/50 transition-all outline-none font-mono"
                        />
                    </div>
                </div>
            </div>
                    ...
                    {signatureData.withSignature.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                            <div className="p-6 rounded-3xl bg-foreground/5 border border-foreground/10">
                                <KeyRound size={32} className="text-on-surface-variant" />
                            </div>
                            <p className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">No se encontraron firmas para esta búsqueda.</p>
                        </div>
                    ) : (
                        viewMode === 'lineal' ? (
                            ...
                            <div className="glass-card-premium rounded-[2.5rem] overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="border-b border-foreground/10 bg-surface-lowest text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">
                                        ...
                                        <tbody className="divide-y divide-foreground/5 text-xs">
                                            {signatureData.withSignature.map((client, idx) => {
                                                ...
                                                return (
                                                    <tr key={client.id} className="hover:bg-foreground/5 transition-colors group">
                                                        ...
                                                        <td className="py-4 px-5">
                                                            ...
                                                            <div className="flex items-center gap-2 mt-0.5">
                                                                <span className="text-[10px] text-on-surface-variant font-bold font-mono">{client.ruc}</span>
                                                                {client.facturadorConfig && (
                                                                    <button
                                                                        ...
                                                                        className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-tertiary/15 hover:bg-tertiary/25 border border-tertiary/30 text-tertiary text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer"
                                                        ...
                                                        <td className="py-4 px-5">
                                                            {client.electronicSignaturePassword ? (
                                                                <div className="inline-flex items-center gap-1.5 bg-surface-lowest px-2.5 py-1 rounded-xl border border-foreground/10">
                                                                    <span className="font-bold text-tertiary min-w-[70px]">
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        title="Copiar clave"
                                                                    >
                                                                        {isCopied ? <Check size={12} className="text-tertiary" /> : <Copy size={12} />}
                                                                    </button>
                                                                </div>
                                                            ) : (
                                                                <span className="text-on-surface-variant italic text-[11px]">Sin clave</span>
                                                            )}
                                                        </td>
                                                        ...
                                                        <td className="py-4 px-5 text-center">
                                                            {isExpired ? (
                                                                ...
                                                                <span className="px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider bg-tertiary/20 text-tertiary border border-tertiary/40 shadow-[0_0_6px_rgba(0,168,150,0.3)]">
                                                                    Válida / Activa
                                                                </span>
                                                            )}
                                                        </td>
                                                        <td className="py-4 px-5 text-right space-x-2">
                                                            {client.signatureFile && (
                                                                <button
                                                                    ...
                                                                    className="px-3 py-1.5 rounded-xl bg-tertiary/15 hover:bg-tertiary/25 text-tertiary text-[10px] font-bold uppercase transition-all border border-tertiary/30 inline-flex items-center gap-1 shadow-sm cursor-pointer"
                                                            ...
                                                            <button
                                                                ...
                                                                className="px-3 py-1.5 rounded-xl bg-tertiary/15 hover:bg-tertiary/25 text-tertiary text-[10px] font-bold uppercase transition-all border border-tertiary/30 inline-flex items-center gap-1 cursor-pointer"
                                                            >
                                                                <PhoneCall size={12} /> WhatsApp
                                                            </button>
                                                            <button
                                                                onClick={() => navigate('clients', { clientIdToView: client.id, initialTab: 'vault' })}
                                                                className="px-3 py-1.5 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-on-surface-variant hover:text-on-surface text-[10px] font-bold uppercase transition-all inline-flex items-center gap-1 border border-foreground/10 cursor-pointer"
                                                            >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                                {signatureData.withSignature.map((client) => {
                                    ...
                                    return (
                                        <div
                                            key={client.id}
                                            className={`glass-card-premium p-6 rounded-[2.5rem] border transition-all duration-300 flex flex-col justify-between gap-4 ${
                                                isExpired
                                        ...
                                        >
                                            <div className="space-y-3">
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className="text-[9px] font-bold uppercase tracking-widest text-on-surface-variant">Verificación .p12</span>
                                                    {isExpired ? (
                                                        ...
                                                        <span className="px-2.5 py-0.5 rounded-full text-[9px] font-bold uppercase bg-tertiary/20 text-tertiary border border-tertiary/40">Válida / Activa</span>
                                                    )}
                                                </div>

                                                <div>
                                                    <h3 className="text-base font-black text-on-surface uppercase tracking-tight font-display line-clamp-1">{client.name}</h3>
                                                    <div className="flex items-center gap-2 mt-0.5">
                                                        <span className="text-xs font-mono font-bold text-on-surface-variant">{client.ruc}</span>
                                                        {client.facturadorConfig && (
                                                            <button
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                ...
                                                <div className="p-3 rounded-2xl bg-surface-lowest border border-foreground/5 space-y-1.5 text-xs">
                                                    <div className="flex justify-between text-on-surface-variant">
                                                        <span>Caducidad:</span>
                                                        <strong className="text-on-surface">{formatExpiry(client.signatureExpirationDate)}</strong>
                                                    </div>
                                                    <div className="flex justify-between text-on-surface-variant">
                                                        <span>Emisor:</span>
                                                        <strong className="text-tertiary truncate max-w-[140px]">{client.signatureProvider || 'SRI Standard'}</strong>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 pt-2 border-t border-foreground/5">
                                                {client.signatureFile && (
                                                    <button
                                                        onClick={() => downloadStoredFile(client.signatureFile)}
                                                        className="py-2 px-3 rounded-xl bg-tertiary/15 hover:bg-tertiary/25 text-tertiary text-xs font-bold uppercase transition-all text-center flex items-center justify-center gap-1.5 border border-tertiary/30 shadow-sm cursor-pointer"
                                                ...
                                                <button
                                                    onClick={() => navigate('clients', { clientIdToView: client.id, initialTab: 'vault' })}
                                                    className="flex-1 py-2 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-on-surface text-xs font-bold uppercase transition-all text-center border border-foreground/10 cursor-pointer"
                                                >
                                                    Ir a Bóveda
                                                </button>
                                            </div>
            ...
            {tab === 'sin-firma' && (
                <div className="glass-card-premium rounded-[2.5rem] p-6 md:p-8 space-y-6">
                    <p className="text-xs text-on-surface-variant font-sans">
                        Mostrando <strong className="text-on-surface">{signatureData.withoutSignature.length}</strong> clientes activos sin archivo de Firma Electrónica (.p12) registrado en su bóveda.
                    </p>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {signatureData.withoutSignature.map((client) => (
                            <div key={client.id} className="p-4 rounded-2xl bg-surface-lowest border border-foreground/10 hover:border-foreground/20 transition-all flex items-center justify-between gap-3 shadow-md">
                                <div>
                                    <p className="text-xs font-bold text-on-surface uppercase tracking-tight font-display">{client.name}</p>
                                    <p className="text-[10px] font-mono text-on-surface-variant">{client.ruc}</p>
                                </div>
                                <button
                                    onClick={() => navigate('clients', { clientIdToView: client.id, initialTab: 'vault' })}
                                    className="px-3 py-1.5 rounded-xl bg-tertiary/15 hover:bg-tertiary/25 text-tertiary border border-tertiary/30 text-[10px] font-bold uppercase transition-all cursor-pointer"
                                >
                                    Cargar Firma
                                </button>
                            </div>
                ...
                <div className="space-y-6">
                    <div className="p-5 rounded-[2.5rem] bg-purple-500/10 border border-purple-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            ...
                            <h3 className="text-base font-black text-on-surface mt-1 font-display">Firmas de Sistemas, Facturadores y Ventas Ocasionales</h3>
                    ...
                    {filteredBackupSignatures.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 gap-4 text-center bg-surface-low rounded-[2.5rem] border border-foreground/10">
                            <div className="p-6 rounded-3xl bg-foreground/5 border border-foreground/10">
                                <Archive size={36} className="text-on-surface-variant" />
                            </div>
                            <p className="text-sm font-bold text-on-surface-variant">No hay firmas guardadas en la Bóveda de Respaldos Externos.</p>
                            <p className="text-xs text-on-surface-variant max-w-sm font-sans">Al usar el Subidor Masivo, puedes seleccionar "Bóveda de Respaldos" para guardar firmas de clientes ocasionales.</p>
                        </div>
                    ) : (
                        <div className="glass-card-premium rounded-[2.5rem] overflow-hidden">
                            <div className="overflow-x-auto">
                                        ...
                                        {filteredBackupSignatures.map((item, idx) => {
                                            ...
                                            return (
                                                <tr key={item.id} className="hover:bg-foreground/5 transition-colors">
                                                    <td className="py-4 px-5 text-center font-bold text-on-surface-variant">{idx + 1}</td>
                                                    <td className="py-4 px-5">
                                                        <span className="font-bold text-on-surface uppercase tracking-tight block text-sm font-display">{item.titular}</span>
                                                        <span className="text-[10px] text-on-surface-variant font-bold block">{item.ruc || 'Sin RUC registrado'}</span>
                                                        <span className="text-[9px] text-on-surface-variant block font-mono">{item.fileName}</span>
                                                    </td>
                                                    <td className="py-4 px-5">
                                                        ...
                                                        {item.password ? (
                                                            <div className="inline-flex items-center gap-1.5 bg-surface-lowest px-2.5 py-1 rounded-xl border border-foreground/10">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="p-1 hover:text-purple-400 text-on-surface-variant transition-colors cursor-pointer"
                                                                    title="Copiar clave"
                                                                >
            ...
            {whatsAppPrompt && (
                <Modal isOpen={true} onClose={() => setWhatsAppPrompt(null)} title="💬 Enviar Recordatorio por WhatsApp" size="md">
                    <div className="space-y-4 p-4 text-on-surface">
                        <p className="text-xs text-on-surface-variant font-sans">
                            Enviarás un mensaje directo al cliente <strong className="text-on-surface">{whatsAppPrompt.clientName}</strong> sobre la caducidad de su firma electrónica:
                        </p>

                        <div className="p-4 rounded-2xl bg-surface-lowest border border-foreground/10 text-xs font-mono text-tertiary whitespace-pre-wrap">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button
                                ...
                                className="px-4 py-2 rounded-xl bg-foreground/5 hover:bg-foreground/10 text-on-surface-variant text-xs font-bold cursor-pointer border border-foreground/10"
                            >
                                Cancelar
                            </button>
                            <a
                                ...
                                rel="noreferrer"
                                onClick={() => setWhatsAppPrompt(null)}
                                className="px-5 py-2 rounded-xl bg-gradient-to-r from-tertiary to-green-600 hover:from-tertiary hover:to-green-500 text-white font-bold text-xs uppercase tracking-wider shadow-lg shadow-tertiary/25 flex items-center gap-1.5 border border-foreground/10 cursor-pointer"
                            >
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-surface-low backdrop-blur-2xl border border-foreground/15 rounded-3xl p-2.5 shadow-[0_20px_50px_rgba(0,0,0,0.8)] flex items-center gap-3 text-on-surface transition-all duration-300">
                ...
                <div className="relative flex items-center">
                    ...
                    <input
                        ...
                        onChange={(e) => setSearchTerm(e.target.value)}
                        placeholder="BUSCAR TITULAR O RUC..."
                        className="pl-8 pr-3 py-1.5 bg-surface-lowest border border-foreground/10 rounded-2xl text-xs text-on-surface placeholder-on-surface-variant focus:outline-none focus:border-tertiary/50 w-44 sm:w-56 font-mono"
                    />
                </div>
                ...
                <button
                    onClick={() => setIsBulkModalOpen(true)}
                    className="px-4 py-2 rounded-2xl bg-gradient-to-r from-tertiary to-tertiary hover:from-tertiary hover:to-tertiary text-white text-xs font-bold uppercase tracking-wider shadow-lg shadow-tertiary/25 flex items-center gap-1.5 shrink-0 transition-all active:scale-95 border border-foreground/10 cursor-pointer"
                >
                    <UploadCloud size={14} />
                ...
                <div className="flex items-center gap-1">
                    <button
                        ...
                        className="p-2 rounded-xl bg-foreground/5 hover:bg-foreground/15 text-on-surface-variant transition-all active:scale-90 cursor-pointer border border-foreground/5"
