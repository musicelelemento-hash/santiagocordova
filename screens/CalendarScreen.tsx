...
export const CalendarScreen: React.FC<CalendarScreenProps> = ({ navigate }) => {
  ...
  return (
    <div className="space-y-6 pb-24 animate-fade-in relative pt-4 sm:pt-0 aurora-premium min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10 px-1 sm:px-0 mb-8">
        <div className="animate-fade-in-left w-full sm:w-auto">
          <div className="flex items-center justify-between sm:justify-start gap-2 mb-2">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 shadow-lg shadow-sky-500/5">
              ...
              <span className="text-[10px] font-black text-sky-500 uppercase tracking-widest">Temporal Intelligence</span>
            </div>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest opacity-50 sm:block hidden">• Scheduler v4.0 Alpha</span>
          </div>
          <h2 className="text-3xl sm:text-5xl font-display font-black text-slate-900 dark:text-white leading-[0.85] tracking-tighter mb-2 italic">
            {safeFormat(currentMonth, 'MMMM').toUpperCase()} <span className="text-gradient-sky font-black italic">{safeFormat(currentMonth, 'yyyy')}</span>
          </h2>
          <div className="flex items-center gap-2 text-slate-500 text-[11px] font-bold uppercase tracking-widest">
            <CalendarIcon size={12} className="text-sky-500" />
        ...
        <div className="flex items-center gap-3 w-full md:w-auto animate-fade-in-right">
          <button
            ...
            className="flex-1 sm:flex-none px-10 py-4 bg-slate-900 dark:bg-white dark:text-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all hover:scale-[1.05] active:scale-[0.95] shadow-xl hover:shadow-sky-500/10 italic"
          >
            Sincronizar Hoy
          </button>
          <div className="flex p-1 bg-white/5 dark:bg-white/5 rounded-[1.5rem] border border-white/10 backdrop-blur-xl">
            <button onClick={handlePrevMonth} className="p-3.5 rounded-xl hover:bg-sky-500/10 text-slate-400 hover:text-sky-500 transition-all group active:scale-90">
              <ChevronLeft size={22} className="group-hover:-translate-x-1 transition-transform" />
            </button>
            <button onClick={handleNextMonth} className="p-3.5 rounded-xl hover:bg-sky-500/10 text-slate-400 hover:text-sky-500 transition-all group active:scale-90">
      ...
      <div className="glass-tactical rounded-[2.5rem] border border-white/5 shadow-2xl relative overflow-hidden flex flex-col group transition-all duration-700 hover:border-sky-500/20">
        <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-sky-500/5 blur-[120px] rounded-full -mr-32 -mt-32 pointer-events-none opacity-50 group-hover:opacity-100 transition-opacity"></div>
        
        {/* Weekday Headers */}
        <div className="grid grid-cols-7 border-b border-white/5 bg-white/5 backdrop-blur-md relative z-10">
          {weekDays.map(day => (
            <div key={day} className="text-center font-black text-[10px] uppercase tracking-[0.3em] py-5 text-slate-500 dark:text-slate-400 italic">
              {day}
            </div>
          ))}
        </div>
            ...
            return (
              <div
                ...
                className={`min-h-[140px] p-3 border-b border-r border-white/5 flex flex-col transition-all duration-500 relative group/day
                  ...
                  ${isCurrentDay ? 'bg-sky-500/[0.03]' : ''}
                  ${dayIdx % 7 === 6 ? 'border-r-0' : ''}`
                }
              >
                {isCurrentDay && (
                  <div className="absolute inset-0 border-2 border-sky-500/20 rounded-[inherit] pointer-events-none animate-pulse"></div>
                )}
                
                <div className="flex justify-between items-start mb-4">
                  <span className={`text-base font-black w-9 h-9 flex items-center justify-center rounded-2xl transition-all duration-500 ${isCurrentDay ? 'bg-sky-500 text-white shadow-[0_0_20px_rgba(14,165,233,0.4)] scale-110 z-10' : isCurrentMonth ? 'text-slate-900 dark:text-white group-hover/day:text-sky-500' : 'text-slate-600'}`}>
                    {safeFormat(day, 'd')}
                  </span>
                  {sriDigit !== null && isCurrentMonth && (
                    <div className="flex flex-col items-end">
                      <span className="text-[8px] font-black text-rose-500 bg-rose-500/10 px-2 py-0.5 rounded-full border border-rose-500/20 uppercase tracking-[0.1em] shadow-lg shadow-rose-500/5 animate-fade-in" title={`Vence 9no dígito: ${sriDigit}`}>
                        DÍGITO {sriDigit}
                      </span>
                    </div>
                  )}
                ...
                <div className="flex-1 space-y-2 overflow-y-auto custom-scrollbar-mini pr-1 relative z-10">
                  {dayEvents.map((event, index) => (
                    ...
                    >
                      <div className="font-black truncate uppercase tracking-tight mb-0.5 group-hover/item:text-sky-500 transition-colors">{event.clientName}</div>
                      <div className="flex items-center gap-1.5 opacity-60 font-bold uppercase tracking-widest text-[8px]">
                        <div className={`w-1 h-1 rounded-full ${event.type === 'task' ? 'bg-sky-500' : 'bg-emerald-500'}`}></div>
      ...
      <div className="flex flex-wrap items-center gap-6 px-8 py-4 glass-tactical border border-white/5 rounded-[2rem] w-fit mx-auto animate-slide-up-fade">
         <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Tareas Operativas</span>
         </div>
         <div className="flex items-center gap-3">
            <div className="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]"></div>
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Protocolos Fiscales</span>
         </div>
         <div className="flex items-center gap-3 border-l border-white/10 pl-6 ml-2">
            <Info size={14} className="text-rose-500" />
            <span className="text-[10px] font-black text-rose-500/70 uppercase tracking-widest italic">Alertas SRI Automatizadas</span>
         </div>
      </div>
    </div>
  );
