...
import { isWeekend, getYear, addDays, getDay, addMonths, subMonths, setMonth, setYear, setDate } from 'date-fns';
    ...
    if (client.regime !== TaxRegime.RimpeNegocioPopular) {
        ...
        if (isMonthly) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                const periodKey = safeFormat(declaredPeriodDate, 'yyyy-MM');
                const periodName = safeFormat(declaredPeriodDate, 'MMMM yyyy');
