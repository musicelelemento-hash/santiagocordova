        ...
        <div className="relative" ref={notificationRef}>
            <button 
                ...
                className={`relative p-3 rounded-2xl transition-all duration-300 border border-white/10 ${
                    isOpen ? 'bg-sky-400 text-white shadow-[0_0_20px_rgba(56,189,248,0.4)]' : 'bg-white/5 hover:bg-white/10 text-slate-400 group'
                } ${alerts.length > 0 && !isOpen ? 'animate-radar-pulse' : ''}`}
            >
                <Bell className={`w-5 h-5 ${isOpen ? 'animate-pulse' : 'group-hover:text-sky-400'}`} />
                {alerts.length > 0 && (
                    <div className="absolute top-2 right-2 w-2.5 h-2.5 bg-rose-400 rounded-full border-2 border-slate-900 shadow-[0_0_8px_rgba(244,63,94,0.6)] animate-pulse"></div>
                )}
            </button>
                                ...
                                {alerts.map(alert => (
                                    ...
                                    >
                                        <div className="flex gap-3">
                                            <div className={`p-2 rounded-lg ${alert.type === 'danger' ? 'bg-rose-400/20 text-rose-400' : 'bg-amber-400/20 text-amber-400'}`}>
