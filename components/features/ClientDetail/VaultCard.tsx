...
export const VaultCard: React.FC<VaultCardProps> = ({ icon: Icon, label, file, onUpload, onDownload, onDelete, isPassword, value, isEditing, onChange }) => {
    ...
    return (
        <div className="bg-white/80 dark:bg-[#051424]/90 backdrop-blur-2xl rounded-3xl p-6 sm:p-8 shadow-xl hover:shadow-2xl border border-slate-200/80 dark:border-white/10 dark:border-t-white/20 hover:border-blue-400/40 dark:hover:border-[#2B6AFF]/50 transition-all duration-500 group relative overflow-hidden">
                        ...
                        <div className="mt-1.5 min-h-[1.75rem] flex items-center">
                            {isPassword ? (
                                isEditing ? (
                                    ...
                                    <p className={`text-[15px] font-black tracking-[0.2em] font-mono ${showPassword ? 'text-slate-900 dark:text-white' : 'text-slate-400 dark:text-slate-500'}`}>
                                        {showPassword ? value : '••••••••••••'}
                                    </p>
                                )
                            ) : file ? (
                ...
                <div className="flex gap-2 relative z-10 font-mono">
                    {file ? (
                        <>
                             <button
                                onClick={onDownload}
                                className="w-10 h-10 flex items-center justify-center bg-slate-100 dark:bg-white/5 hover:bg-[#2B6AFF] hover:text-white text-slate-600 dark:text-slate-300 rounded-xl transition-all active:scale-95 border border-slate-200 dark:border-white/10 shadow-sm"
                                title="Descargar"
                            >
                            ...
                            <button 
                                onClick={onDelete}
                                className="w-10 h-10 flex items-center justify-center bg-slate-100 dark:bg-white/5 hover:bg-rose-500 hover:text-white text-slate-600 dark:text-slate-300 rounded-xl transition-all active:scale-95 border border-slate-200 dark:border-white/10 shadow-sm" 
                                title="Eliminar"
                            >
