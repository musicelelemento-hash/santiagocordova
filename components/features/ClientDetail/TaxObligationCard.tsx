...
export const TaxObligationCard: React.FC<TaxObligationCardProps> = ({
    ...
    const renderIvaRentaSteps = () => (
        <div className="space-y-4 relative z-10">
            {/* Step: Declaration */}
            <div className={`flex flex-col md:flex-row items-center justify-between gap-6 p-6 rounded-[2rem] bg-white dark:bg-surface-low/30 border border-slate-100 dark:border-white/5 transition-all duration-700 group/step ${overdueStatus && !isDeclared ? 'border-rose-200 dark:border-rose-500/30 bg-rose-50/30 dark:bg-rose-500/5' : 'hover:border-primary/30 dark:hover:border-primary/40 shadow-sm hover:shadow-md'}`}>
                <div className="flex items-center gap-5 w-full md:w-auto">
                    ...
                    <div className="flex flex-col min-w-0">
                        <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] font-premium leading-none">FASE 01</span>
                ...
                {isDeclared ? (
                    <div className="flex items-center gap-3 text-blue-600 dark:text-primary-low bg-blue-50 dark:bg-primary/20 px-5 py-2.5 rounded-xl border border-blue-100 dark:border-primary/30 transition-all font-premium shadow-sm">
            ...
            {/* Step: Fees */}
            <div className={`flex flex-col md:flex-row items-center justify-between gap-6 p-6 rounded-[2rem] bg-white dark:bg-surface-low/30 border border-slate-100 dark:border-white/5 transition-all duration-700 group/step hover:border-emerald-200 dark:hover:border-emerald-500/40 shadow-sm hover:shadow-md ${isPaid ? 'bg-emerald-50/30 dark:bg-emerald-500/5 border-emerald-100 dark:border-emerald-500/20' : ''}`}>
                <div className="flex items-center gap-5 w-full md:w-auto">
                    <div className={`w-3 h-3 rounded-full transition-all duration-1000 flex-shrink-0 ${isPaid ? 'bg-emerald-500 scale-110 shadow-[0_0_20px_rgba(16,185,129,0.3)]' : 'bg-slate-200 dark:bg-slate-700'}`}></div>
                    <div className="flex flex-col min-w-0">
                        <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] font-premium leading-none">FASE 02</span>
                        <span className="text-[11px] font-black text-slate-900 dark:text-slate-200 uppercase mt-1.5 font-premium tracking-tight">LIQUIDACIÓN</span>
                    </div>
                </div>
                {isPaid ? (
                    <div className="flex items-center justify-between w-full md:w-auto gap-6 text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-6 py-2.5 rounded-2xl border border-emerald-100 dark:border-emerald-500/20 group/revert font-premium shadow-sm">
                        <span className="text-sm font-black tracking-tight">${amount.toFixed(2)}</span>
                        {onRevertPayment && (
                            <button 
                                onClick={(e) => { e.stopPropagation(); onRevertPayment(); }} 
                                className="p-2 opacity-0 group-hover/revert:opacity-100 hover:bg-rose-100 dark:hover:bg-rose-500/20 text-rose-500 rounded-xl transition-all active:scale-90 shadow-sm" 
                            >
    ...
    const renderRefundSteps = () => (
        <div className="space-y-6 relative z-10">
            <div className="flex flex-col md:flex-row items-center justify-between p-8 bg-white dark:bg-surface-low/30 rounded-[2.5rem] border border-slate-100 dark:border-white/5 gap-8 shadow-sm hover:shadow-md transition-all duration-700">
                <div className="flex items-center gap-6 w-full">
                    ...
                    <div className="min-w-0">
                        <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] font-premium">ESTADO TÉCNICO</div>
                ...
                <div className="flex gap-4 w-full md:w-auto">
                    {status === 'Pendiente' && (
                        <button onClick={() => onAction?.('start')} className="w-full md:w-auto px-10 py-4 bg-slate-900 dark:bg-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-primary dark:hover:bg-primary-low shadow-sm hover:shadow-md active:scale-95 transition-all font-premium">INICIAR PROCESO</button>
                    )}
                    {status === 'Solicitado' && (
            ...
            {resolutionFile && (
                <div className="flex items-center justify-between p-6 bg-white dark:bg-surface-low/30 rounded-[2rem] border border-slate-100 dark:border-white/5 hover:border-emerald-200 dark:hover:border-emerald-500/40 hover:bg-emerald-50/20 dark:hover:bg-emerald-500/5 transition-all cursor-pointer group shadow-sm hover:shadow-md">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <div className="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] font-premium">ORDEN DE RESOLUCIÓN</div>
                    ...
                    <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-500 group-hover:bg-emerald-500 dark:group-hover:bg-emerald-500 group-hover:text-white transition-all shadow-sm">
            ...
            {!resolutionFile && !isCompleted && onUpload && (
                <button onClick={onUpload} className="w-full flex flex-col items-center justify-center gap-4 p-12 border-2 border-dashed border-slate-200 dark:border-white/10 text-slate-400 dark:text-slate-500 hover:border-blue-400/40 dark:hover:border-primary/40 hover:bg-blue-50/30 dark:hover:bg-primary/5 hover:text-blue-600 dark:hover:text-primary-low rounded-[3rem] transition-all group font-premium shadow-sm hover:shadow-inner">
                    ...
                    <div className="flex flex-col items-center gap-1">
                        ...
                        <span className="text-[9px] font-bold text-slate-300 dark:text-slate-600 tracking-widest uppercase">SOLO PDF AUTORIZADO</span>
                    </div>
                </button>
            )}
        </div>
        ...
        <div className={`p-8 sm:p-14 rounded-[3rem] shadow-architect hover:shadow-2xl transition-all duration-1000 group overflow-hidden relative border ${isCompleted ? 'bg-white dark:bg-surface/40 backdrop-blur-3xl border-emerald-200 dark:border-emerald-500/30 shadow-emerald-50 dark:shadow-emerald-500/10' : 'bg-white dark:bg-surface/40 backdrop-blur-3xl border-slate-100 dark:border-white/10 hover:border-blue-200 dark:hover:border-primary/30'}`}>
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-10 mb-14 relative z-10">
                <div className="flex items-start sm:items-center gap-8 min-w-0 flex-1">
                    <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-[1.5rem] flex items-center justify-center flex-shrink-0 transition-all duration-1000 shadow-sm ${isCompleted ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20' : 'bg-slate-50 dark:bg-slate-800 text-slate-400 dark:text-slate-500 group-hover:text-blue-600 dark:group-hover:text-primary group-hover:bg-blue-50 dark:group-hover:bg-primary/10 border border-slate-100 dark:border-white/5'}`}>
                    ...
                    <div className="min-w-0 flex-1">
                        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] font-premium mb-2.5">{period ? formatPeriodForDisplay(period) : 'PROTOCOLOS ESPECIALES'}</p>
                        <h4 className="font-premium font-black text-slate-950 dark:text-slate-50 text-2xl sm:text-5xl tracking-tight leading-none uppercase truncate">{title}</h4>
                    </div>
                </div>
            ...
            {isIvaOrRenta && !isPaid && (
                <div className="mt-14 pt-12 border-t border-slate-100 dark:border-white/5 flex gap-4 relative z-10">
                    <button onClick={onDeclare} className="flex-grow py-6 bg-slate-900 dark:bg-primary text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.4em] transition-all shadow-xl shadow-slate-100 dark:shadow-primary/20 hover:bg-primary dark:hover:bg-primary-low active:scale-95 text-center font-premium hover:shadow-blue-100">
                        EJECUTAR EXPEDIENTE
                    </button>
                    <button onClick={onUpload} className="p-6 bg-white dark:bg-surface-low/50 border border-slate-100 dark:border-white/5 rounded-2xl text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-primary hover:border-blue-200 dark:hover:border-primary/30 transition-all duration-700 active:scale-95 flex-shrink-0 shadow-sm hover:shadow-md">
