...
export const ClientDetailView: React.FC<ClientDetailViewProps> = memo(({ client, onSave, onBack, serviceFees, sriCredentials }) => {
                ...
                <div className={`p-6 sm:p-8 rounded-3xl sm:rounded-[2.5rem] border ${isWorkOrder ? 'bg-amber-500/10 border-amber-500/20 shadow-[0_0_30px_rgba(245,158,11,0.1)]' : (isFullyAlDia ? 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_0_30px_rgba(16,185,129,0.1)]' : 'bg-slate-950/40 border-white/5')} relative overflow-hidden group transition-all duration-700`}>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="mt-6 flex items-center gap-3">
                            <div className={`w-2 h-2 rounded-full animate-ping ${isFullyAlDia ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                            ...
                            <div className="space-y-8">
                                ...
                                {client.taxProfile?.requiresAnnualRenta && editedClient.rentaRefundStatus !== 'Completado' && (
                                    <div className="p-8 bg-slate-900/60 rounded-[2.5rem] border border-white/5 space-y-8 relative overflow-hidden group/renta shadow-2xl">
                                            ...
                                            {[
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`flex items-center gap-3 px-5 py-3 rounded-2xl border transition-all whitespace-nowrap ${isActive ? 'bg-cyan-500 text-slate-950 border-cyan-400 shadow-[0_0_25px_rgba(6,182,212,0.4)]' : (isDone ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-slate-950 text-slate-600 border-white/5 hover:border-white/10')}`}
                                                    >
                                        ...
                                        <TaxObligationCard
                                            title={`REEMBOLSO RENTA (${editedClient.rentaRefundStatus || 'INACTIVO'})`}
                                    ...
                                    <div className="p-8 bg-slate-900/60 rounded-[2.5rem] border border-white/5 space-y-8 relative overflow-hidden group/elderly shadow-2xl">
                                        ...
                                        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                                            {[
                                                ...
                                                { id: 'En Proceso', label: 'Procesando', icon: Clock, action: 'process' },
                                                { id: 'Completado', label: 'Finalizado', icon: ShieldCheck, action: 'complete' }
                                            ].map((step) => {
                                        ...
                                        <TaxObligationCard
                                            title={`DEVOLUCIÓN T. EDAD (${editedClient.elderlyDevolucionIvaStatus || 'PENDIENTE'})`}
                                            period={complianceStats?.iva.period || ''}
                                            status={
                                            ...
                                            onPay={() => {
                                                ...
                                                setEditedClient(updated);
                                                onSave(updated);
                                                toast.success("Pago T.EDAD registrado");
                                            }}
                    ...
                    <div className="bg-slate-950/60 backdrop-blur-xl rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group">
                        ...
                        <div className="space-y-10">
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center text-cyan-400 shadow-2xl group-hover:scale-110 transition-transform duration-700 shadow-inner">
                                    <Mail size={28} />
                                </div>
                            ...
                            <div className="flex items-center gap-6">
                                <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center text-amber-400 shadow-2xl group-hover:scale-110 transition-transform duration-700 shadow-inner">
                                    <Phone size={28} />
                                </div>
                                <div className="flex-1 min-w-0">
                        ...
                        <div className="grid grid-cols-2 gap-5 mt-14 pt-10 border-t border-white/5">
                            <button onClick={handleOpenSRI} className="flex flex-col items-center gap-4 p-5 bg-slate-900/60 hover:bg-cyan-500/10 border border-white/5 hover:border-cyan-500/30 rounded-[2rem] transition-all group/btn">
                                <Globe size={24} className="text-cyan-400 group-hover/btn:scale-125 transition-all duration-500" />
                                <span className="text-[9px] font-black text-slate-500 group-hover/btn:text-cyan-400 uppercase tracking-[0.2em]">Portal SRI</span>
                            </button>
                            <button onClick={handleWhatsApp} className="flex flex-col items-center gap-4 p-5 bg-slate-900/60 hover:bg-emerald-500/10 border border-white/5 hover:border-emerald-500/30 rounded-[2rem] transition-all group/btn">
                                <MessageCircle size={24} className="text-emerald-400 group-hover/btn:scale-125 transition-all duration-500" />
            ...
            <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-12">
                    <div>
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mt-3">Registro histórico de despliegues y validaciones</p>
                    </div>
                </div>
            ...
            <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group">
                ...
                <div className="flex items-center justify-between mb-10">
                    <h3 className="text-lg font-black text-white tracking-tight uppercase flex items-center gap-3">
                        <FileClock className="text-emerald-400" size={24} />
    ...
    const renderVaultTab = () => (
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[1px] bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent"></div>
                ...
                <div className="flex items-center justify-between mb-12">
                    <div>
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mt-3">Sustento documental de operaciones fiscales</p>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                    <button
                        ...
                        className="aspect-square rounded-[2.5rem] border-2 border-dashed border-white/10 flex flex-col items-center justify-center gap-4 hover:border-cyan-500/50 hover:bg-cyan-500/5 transition-all group relative overflow-hidden"
                    >
                        <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        <div className="w-16 h-16 rounded-2xl bg-slate-900 border border-white/5 flex items-center justify-center group-hover:scale-110 transition-transform shadow-2xl relative z-10">
                            <Plus className="text-slate-500 group-hover:text-cyan-400" size={32} />
                        </div>
    ...
    const renderSettingsTab = () => (
        <div className="space-y-10 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-5 duration-700 pb-20">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group shadow-2xl">
                    <div className="absolute -top-10 -right-10 w-48 h-48 bg-cyan-500/5 blur-[80px] pointer-events-none"></div>
                    <h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.4em] mb-10 flex items-center gap-4">
                    ...
                    <div className="space-y-8">
                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                                Honorario {editedClient.taxProfile?.ivaFrequency === 'Semestral' ? 'Semestral' : 'Mensual'} (IVA)
                            </label>
                            <div className="relative group/input">
                                <div className="absolute left-6 top-1/2 -translate-y-1/2 text-cyan-400 font-black">$</div>
                                <input
                                    type="number"
                                    value={monthlyFee}
                                    onChange={(e) => setMonthlyFee(e.target.value)}
                                    disabled={!isEditing}
                                    className="w-full pl-12 pr-6 py-5 bg-slate-950/60 border border-white/5 rounded-2xl text-base font-black text-white focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all disabled:opacity-40 outline-none shadow-inner"
                                />
                            </div>
                        </div>

                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Protocolo Anual (Renta)</label>
                            <div className="relative group/input">
                                <div className="absolute left-6 top-1/2 -translate-y-1/2 text-amber-500 font-black">$</div>
                                <input
                                    type="number"
                                    value={annualFee}
                                    onChange={(e) => setAnnualFee(e.target.value)}
                                    disabled={!isEditing}
                                    className="w-full pl-12 pr-6 py-5 bg-slate-950/60 border border-white/5 rounded-2xl text-base font-black text-white focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500/50 transition-all disabled:opacity-40 outline-none shadow-inner"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group shadow-2xl">
                    <div className="absolute -bottom-10 -left-10 w-48 h-48 bg-emerald-500/5 blur-[80px] pointer-events-none"></div>
                    <h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.4em] mb-10 flex items-center gap-4">
                    ...
                    <div className="space-y-8">
                        <div className="space-y-4">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Régimen Estratégico</label>
                            <select
                                value={editedClient.regime}
                                onChange={(e) => setEditedClient({ ...editedClient, regime: e.target.value as TaxRegime })}
                                disabled={!isEditing}
                                className="w-full px-6 py-5 bg-slate-950/60 border border-white/5 rounded-2xl text-sm font-black text-white focus:ring-2 focus:ring-cyan-500/50 transition-all disabled:opacity-40 outline-none appearance-none shadow-inner"
                            >
            ...
            <div className="bg-slate-950/40 backdrop-blur-md rounded-[3rem] p-10 border border-white/5 relative overflow-hidden group shadow-2xl">
                <h3 className="text-sm font-black text-slate-500 uppercase tracking-[0.4em] mb-10 flex items-center gap-4">
                    ...
                    Protocolos de Comunicación y Ubicación
                </h3>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Vector Email</label>
                        <input
                            type="email"
                            value={editedClient.email || ''}
                            onChange={(e) => setEditedClient({ ...editedClient, email: e.target.value })}
                            disabled={!isEditing}
                            className="w-full px-6 py-5 bg-slate-950/60 border border-white/5 rounded-2xl text-sm font-black text-white focus:ring-2 focus:ring-cyan-500/50 transition-all disabled:opacity-40 outline-none shadow-inner"
                            placeholder="OPERATIVO@SANTIAGOCORDOVA.COM"
                        />
                    </div>
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Enlace Móvil</label>
                        <input
                            type="text"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-8">
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Frecuencia IVA</label>
                        <select
                    ...
                    <div className="space-y-4">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Encriptación SRI</label>
                        <div className="relative">
                            <input
                                ...
                                placeholder="********"
                            />
                            <button onClick={() => setPasswordVisible(!passwordVisible)} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-600 hover:text-cyan-400">
                ...
                <div className="mt-8 space-y-4">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Geolocalización Matriz</label>
                    <input
                        type="text"
                        ...
                        disabled={!isEditing}
                        className="w-full px-6 py-5 bg-slate-950/60 border border-white/5 rounded-2xl text-sm font-black text-white focus:ring-2 focus:ring-cyan-500/50 transition-all disabled:opacity-40 outline-none shadow-inner"
                        placeholder="COORDENADAS DE MATRIZ"
                    />
                </div>

                <div className="mt-8 space-y-4">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Inteligencia Económica (Actividad)</label>
                    <textarea
                        value={editedClient.economicActivity || ''}
                        onChange={(e) => setEditedClient({ ...editedClient, economicActivity: e.target.value })}
                        disabled={!isEditing}
                        rows={3}
                        className="w-full px-6 py-5 bg-slate-950/60 border border-white/5 rounded-[2rem] text-sm font-black text-white focus:ring-2 focus:ring-cyan-500/50 transition-all disabled:opacity-40 outline-none shadow-inner resize-none"
                        placeholder="DEFINICIÓN DE OPERACIONES"
                    />
                </div>
            </div>
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-slate-950/90 backdrop-blur-sm overflow-hidden animate-in fade-in duration-500">
            <div className="bg-slate-900/40 backdrop-blur-md w-full h-full md:max-h-[90vh] md:max-w-6xl md:rounded-3xl shadow-2xl flex flex-col relative overflow-hidden border border-white/10 aurora-premium">
                {/* Tactical HUD accents */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[400px] h-[1px] bg-gradient-to-r from-transparent via-cyan-500/30 to-transparent"></div>
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[300px] h-[1px] bg-gradient-to-r from-transparent via-amber-500/20 to-transparent"></div>

                <button onClick={onBack} className="absolute top-8 right-8 z-50 p-3 bg-slate-950/50 border border-white/10 text-slate-500 hover:text-cyan-400 rounded-2xl transition-all md:flex hidden hover:scale-110 active:scale-90 group/close shadow-2xl">
                    <X size={20} className="group-hover:rotate-90 transition-transform duration-500" />
                </button>
                <div className="flex-1 overflow-y-auto custom-scrollbar relative">
                    {/* Background decorative elements */}
                    <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-cyan-500/5 blur-3xl rounded-full -mr-32 -mt-32 pointer-events-none"></div>
                    <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-indigo-500/5 blur-3xl rounded-full -ml-32 -mb-32 pointer-events-none"></div>
                    ...
                    <div className="p-6 md:p-10 pb-20 relative z-10">
                        ...
                        />
                        {/* Tactical Hub Navigation */}
                        <div className="flex gap-1.5 sm:gap-2 p-1.5 sm:p-2 bg-slate-950/60 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-white/10 mb-8 sm:mb-10 overflow-x-auto no-scrollbar max-w-full sm:max-w-fit mx-auto md:mx-0 shadow-xl">
                            {(['profile', 'history', 'vault', 'settings'] as const).map(tab => (
                                <button
                                    key={tab}
                                    onClick={() => setActiveTab(tab)}
                                    className={`px-4 sm:px-8 py-2.5 sm:py-3 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] transition-all relative rounded-xl group/tab whitespace-nowrap ${activeTab === tab ? 'text-white bg-cyan-500/20 shadow-[inset_0_0_25px_rgba(6,182,212,0.15)]' : 'text-slate-500 hover:text-slate-200 hover:bg-white/5'}`}
                                >
                                    <span className="relative z-10 flex items-center gap-3">
                                        {tab === 'profile' && <LayoutDashboard size={16} className={activeTab === tab ? 'text-cyan-400' : 'group-hover/tab:text-cyan-400/50'} />}
                                        {tab === 'history' && <Activity size={16} className={activeTab === tab ? 'text-cyan-400' : 'group-hover/tab:text-cyan-400/50'} />}
                                        {tab === 'vault' && <Lock size={16} className={activeTab === tab ? 'text-cyan-400' : 'group-hover/tab:text-cyan-400/50'} />}
                                        {tab === 'settings' && <Settings size={16} className={activeTab === tab ? 'text-cyan-400' : 'group-hover/tab:text-cyan-400/50'} />}
                                        {tab === 'profile' ? 'Estrategia' : tab === 'history' ? 'Operativas' : tab === 'vault' ? 'Bóveda' : 'Sistemas'}
                                    </span>
                                    {activeTab === tab && (
                                        <div className="absolute inset-0 border border-cyan-500/40 rounded-2xl animate-pulse shadow-[0_0_15px_rgba(6,182,212,0.2)]"></div>
                                    )}
                                </button>
                            ))}
                        </div>
                ...
                <div className="flex-none p-6 md:px-10 bg-slate-950/60 backdrop-blur-lg border-t border-white/5 flex justify-between items-center relative z-20">
                    <div className="flex items-center gap-6">
                    ...
                    <div className="flex gap-4">
                        {isEditing ? (
                            <>
                                <button onClick={() => !isAnalyzingPdf && fileInputRef.current?.click()} className="px-4 py-3 bg-amber-500/10 hover:bg-amber-500/20 text-amber-400 rounded-xl border border-amber-500/20 text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 transition-all shadow-lg hover:shadow-amber-500/10">
                                    {isAnalyzingPdf ? <Loader size={12} className="animate-spin" /> : <ScanLine size={14} />}
                                    <span>Scanner <span className="hidden sm:inline">SRI</span></span>
                                </button>
                                <button onClick={() => setIsEditing(false)} className="px-5 py-3 bg-slate-900 border border-white/5 text-slate-500 rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest hover:text-slate-300 transition-all">Cancel</button>
                                <button onClick={handleSave} className="px-6 sm:px-8 py-3 bg-gradient-to-r from-cyan-600 to-cyan-500 text-white rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] shadow-[0_0_25px_rgba(6,182,212,0.3)] hover:shadow-cyan-500/50 transition-all active:scale-95">Guardar</button>
                            </>
                        ) : (
                            <button onClick={() => setIsEditing(true)} className="w-full sm:w-auto px-8 sm:px-10 py-3 bg-white text-slate-950 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] shadow-[0_10px_30px_rgba(255,255,255,0.1)] hover:bg-cyan-400 hover:text-white transition-all transform hover:-translate-y-1 active:scale-95">Módulo de Edición</button>
                        )}
                    </div>
                </div>
                ...
                <Modal isOpen={!!confirmation} onClose={() => setConfirmation(null)} title="Confirmar Acción">
                    <div className="p-6 text-center">
                        ...
                        <button onClick={() => handleConfirmAction(false)} className="w-full py-4 bg-brand-navy text-white font-black rounded-2xl text-[10px] uppercase tracking-widest">Confirmar</button>
                    </div>
                </Modal>
                        ...
                        <div className="p-4 bg-white rounded-xl">
                            <div ref={receiptRef} className="text-center font-mono text-sm space-y-3 mb-6">
                                ...
                                <div className="p-3 bg-slate-50 rounded-lg">
                                    ...
                                    <p className="font-bold text-lg">${receiptData.totalAmount.toFixed(2)}</p>
                                </div>
                            </div>
                            <div className="flex gap-3">
                                <button onClick={handlePrintReceipt} className="flex-1 bg-brand-navy text-white py-3 rounded-xl font-bold">Imprimir</button>
                                <button onClick={copyReceiptToClipboard} className="flex-1 bg-slate-100 text-slate-700 py-3 rounded-xl font-bold">Copiar</button>
                            </div>
                        </div>
                    )}
                </Modal>
                <Modal isOpen={!!mismatchData} onClose={() => setMismatchData(null)} title="RUC Erróneo">
                    <div className="text-center p-4">
                        ...
                        <button onClick={handleJumpToOwner} className="w-full py-4 bg-brand-navy text-white font-black rounded-2xl mt-4">Ver Ficha Correcta</button>
                    </div>
                </Modal>
            </div>
        </div>
