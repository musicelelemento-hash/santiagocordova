...
export const ClientDetailView: React.FC<ClientDetailViewProps> = memo(({ client, onSave, onBack, serviceFees, sriCredentials, initialTab }) => {
    ...
    return (
        <div className="w-full h-full bg-slate-50 dark:bg-[#020b14] border-none dark:border-white/5 md:rounded-[2.5rem] flex flex-col relative overflow-hidden group/modal shadow-[0_32px_64px_-12px_rgba(0,0,0,0.4)] animate-in fade-in duration-700">
                ...
                <div className="flex items-center gap-1.5 p-1.5 bg-white/90 dark:bg-[#051424]/90 backdrop-blur-[40px] border border-slate-200 dark:border-white/10 dark:border-t-white/20 rounded-[2.5rem] shadow-2xl shadow-black/40 pointer-events-auto ring-1 ring-black/[0.05] dark:ring-white/[0.05]">
                    {(['profile', 'metrics', 'history', 'vault', 'settings'] as const).map((tab) => (
                        <button
                            ...
                            className={`group relative flex items-center gap-3 px-5 py-3 rounded-[2rem] transition-all duration-500 overflow-hidden ${activeTab === tab
                                    ? (isDark
                                        ? 'bg-gradient-to-r from-[#00A896] to-teal-600 text-white shadow-lg shadow-[#00A896]/30 scale-[1.05] -translate-y-0.5 border border-white/20'
                                        : 'bg-slate-900 text-white shadow-xl shadow-slate-200 scale-[1.05] -translate-y-0.5')
                                    : (isDark
                        ...
                        >
                            <div className="relative z-10 flex items-center gap-2">
                                ...
                                {tab === 'settings' && <Settings size={15} className={`transition-all duration-500 ${activeTab === tab ? 'rotate-0' : 'group-hover:rotate-90 group-hover:scale-110'}`} />}
                    ...
                    <button
                        onClick={onBack}
                        className="hidden md:flex items-center gap-2.5 px-5 py-3 text-slate-400 hover:text-rose-400 transition-all duration-500 group rounded-[2rem] hover:bg-rose-500/10"
            ...
            <button onClick={onBack} className="absolute top-8 right-8 z-50 p-3.5 bg-white/80 dark:bg-[#051424]/90 backdrop-blur-xl border border-slate-200 dark:border-white/10 text-slate-400 hover:text-rose-400 rounded-2xl transition-all md:flex hidden hover:scale-105 active:scale-95 group/close shadow-xl hover:border-rose-500/30">
                <X size={20} className="group-hover:rotate-90 transition-transform duration-500" />
            </button>

            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
                {/* LEFT COLUMN - Profile Header (Static/Scroll-independent on Desktop) */}
                <div ref={leftColRef} className="w-full lg:w-[330px] xl:w-[370px] flex-none border-b lg:border-b-0 lg:border-r border-slate-200/50 dark:border-white/10 bg-slate-50/50 dark:bg-[#051424]/60 p-6 lg:p-8 overflow-y-auto no-scrollbar">
                    <ClientHeader
                        client={client}
                        onBack={onBack}
                        totalDebt={totalDebt}
