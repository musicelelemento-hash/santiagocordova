...
const TableRow = memo(({ data, index, style }: ListChildComponentProps<VirtualClientTableProps>) => {
    ...
    return (
        <div style={style} className={`flex border-b border-slate-100 dark:border-white/5 hover:bg-sky-500/[0.02] dark:hover:bg-sky-500/[0.05] transition-all items-center px-4 group/row hover:shadow-[inset_4px_0_0_0_#0ea5e9] ${isRefundAlertActive ? 'animate-heartbeat ring-2 ring-inset ring-sky-500/50' : ''}`}>
            {/* Medalla Elite */}
            <div className="w-16 shrink-0 flex flex-col items-center group cursor-help relative" title={merit.label}>
                <div className={`p-2 rounded-xl bg-opacity-10 ${merit.rank === 1 ? 'bg-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : (merit.rank === 2 ? 'bg-slate-400' : 'bg-orange-600')} transition-transform group-hover/row:scale-110`}>
                    <merit.icon size={22} className={`${merit.color}`} strokeWidth={merit.rank === 1 ? 3 : 2} />
                </div>
                <span className={`text-[7px] font-black uppercase mt-1.5 px-1.5 py-0.5 rounded border ${merit.color} ${merit.rank === 1 ? 'border-amber-500/30' : 'border-slate-500/20'}`}>{merit.label.split(' - ')[0]}</span>
            </div>
            ...
            <div className="flex-1 min-w-0 px-4">
                <div className="flex items-center gap-2">
                    ...
                    <span 
                        className="font-black text-slate-800 dark:text-white text-[13px] truncate group-hover/row:text-sky-600 transition-colors uppercase tracking-tight" 
                        title={client.name}
                    >
                        {client.tradeName || client.name}
                    </span>
                    {client.isActive === false && (
                        <span className="text-[7px] font-black bg-rose-500 text-white px-1.5 py-0.5 rounded-full uppercase flex items-center gap-1">
                    ...
                    {(client.hasElderlyDevolucionIva || (client as any).hasElderlyIvaRefund) && (
                        <div className="flex items-center gap-1 text-[7px] font-black bg-emerald-500 text-white px-2 py-0.5 rounded-full uppercase tracking-tighter ring-2 ring-emerald-500/20">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 truncate uppercase tracking-wider" title={client.name}>
                        {client.tradeName ? client.name : client.ruc}
                    </span>
                    {client.tradeName && (
                        <span className="font-mono text-[8px] text-slate-400 bg-slate-100 dark:bg-white/5 px-1.5 py-0.5 rounded border border-slate-200 dark:border-white/10 uppercase tracking-widest">{client.ruc}</span>
                    )}
                    <span className="text-[8px] font-black text-sky-600/70 uppercase tracking-wider font-mono">{client.regime}</span>
                </div>
            </div>
            ...
            <div className="w-40 shrink-0 px-4 border-l border-slate-100 dark:border-white/5 h-full flex flex-col justify-center">
                <div className="flex flex-wrap gap-1.5">
                    {needsIva && (
                        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border transition-all ${isCampaignDone ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' : 'bg-slate-100 dark:bg-white/5 border-slate-200 dark:border-white/10 text-slate-400'}`}>
                            {isCampaignDone ? <LucideIcons.ShieldCheck size={10} strokeWidth={3} /> : <LucideIcons.Loader2 size={10} className="animate-spin" />}
                            <span className="text-[9px] font-black uppercase tracking-widest">{frequencyText.replace('IVA ', '')}</span>
                        </div>
                    )}
                    {needsRenta && (
                        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border transition-all ${isRentaDeclared ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-600' : 'bg-orange-500/10 border-orange-500/20 text-orange-600'}`}>
                            {isRentaDeclared ? <LucideIcons.Award size={10} strokeWidth={3} /> : <LucideIcons.Target size={10} />}
                            <span className="text-[9px] font-black uppercase tracking-widest text-inherit">RENTA</span>
                        </div>
                    )}
                    {client.hasRentaRefund && (
                        <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border transition-all ${client.rentaRefundStatus === 'Completado' ? 'bg-sky-500/10 border-sky-500/20 text-sky-600' : 'bg-rose-500/10 border-rose-500/20 text-rose-500 animate-pulse'}`}>
                            <LucideIcons.HandCoins size={10} />
                            <span className="text-[9px] font-black uppercase tracking-widest">DEV. RENTA</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="w-44 shrink-0 px-4 border-l border-slate-100 dark:border-white/5 h-full flex items-center">
                <div className={`flex items-center gap-3 p-2 rounded-xl border w-full transition-all ${isCampaignDone ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-slate-50/50 dark:bg-white/5 border-slate-200 dark:border-white/10'}`}>
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isCampaignDone ? 'bg-emerald-500 text-white' : (needsIva && ivaDueDate && isPast(ivaDueDate) ? 'bg-rose-500 text-white animate-pulse' : 'bg-sky-500 text-white')}`}>
                    ...
                    {(campaignDecl?.proof_file || decl?.proof_file) && (
                        <button 
                            ...
                            className="ml-auto p-1.5 rounded-lg bg-sky-500/10 text-sky-500 hover:bg-sky-500 hover:text-white transition-all border border-sky-500/20" 
            ...
            <div className="w-28 shrink-0 px-4 border-l border-slate-100 dark:border-white/5 h-full flex flex-col justify-center">
                <span className="font-black text-emerald-600 dark:text-emerald-400 text-base font-mono leading-none tracking-tighter">${fee.toFixed(2)}</span>
                <span className="text-[8px] text-slate-400 font-black uppercase tracking-widest mt-1">Recurrencia</span>
            </div>
            ...
            <div className="w-52 shrink-0 px-4 border-l border-slate-100 dark:border-white/5 h-full flex flex-col justify-center gap-1.5">
                <button
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border
                    ${isCampaignPaid ? 'bg-emerald-500 text-white border-emerald-400 shadow-lg shadow-emerald-500/20' : 'bg-[#0B2149] text-white border-white/10 hover:bg-sky-900 active:scale-95'}`}>
                ...
                <button 
                    ...
                    className={`flex items-center justify-center gap-2 px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border
                    ${isCampaignDone ? 'bg-slate-100 dark:bg-white/5 text-slate-400 border-slate-200 dark:border-white/10' : 'bg-sky-500 text-white border-sky-400 hover:bg-sky-600 shadow-lg shadow-sky-500/20'}`}>
            ...
            <div className="w-32 shrink-0 px-4 flex items-center justify-end gap-2">
                <button 
                    ...
                    className="flex items-center justify-center gap-2 text-white font-black text-[10px] bg-slate-800 dark:bg-white/5 hover:bg-sky-500 dark:hover:bg-sky-500 px-4 py-2.5 rounded-xl transition-all border border-white/5 uppercase tracking-widest"
                >
                    Ficha
...
export const VirtualClientTable: React.FC<VirtualClientTableProps> = (props) => {
    return (
        <div className="bg-white/40 dark:bg-slate-900/60 backdrop-blur-xl rounded-[24px] shadow-2xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col" style={{ height: 'calc(100vh - 420px)', minHeight: '550px' }}>
            <div className="flex bg-slate-100/50 dark:bg-black/20 text-[10px] font-black uppercase text-slate-500 py-4 px-4 border-b border-slate-200 dark:border-white/5 active:select-none tracking-widest">
