    ...
    return (
        <div className="p-4 bg-white dark:bg-gray-900 rounded-lg animate-slide-up-fade relative" style={{ opacity: 0 }}>
            ...
            {(task as any).isAutoGenerated && (
                <div className="p-4 mb-6 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl flex items-center justify-between">
                    <div>
                        <p className="text-amber-900 dark:text-amber-100 font-semibold text-sm uppercase tracking-tight">Órden de Trabajo Táctica</p>
                        <p className="text-amber-700 dark:text-amber-400 text-xs">Esta tarea se genera automáticamente basada en el historial de pagos.</p>
                    </div>
                    {task.clientId && (
                        <button
                            className="px-4 py-2 bg-amber-400 text-white text-[10px] font-semibold uppercase tracking-widest rounded-xl shadow-lg shadow-amber-400/20"
                            onClick={() => {
                                // How to navigate to client profile? Usually requires a state change in the parent
                                // For now, we'll assume the back button is enough or we could add a prop
                            }}
                        >
                            Ver Perfil Completo
                        </button>
            ...
            <div className="space-y-4">
                    ...
                    {isEditing ? (
                        <>
                            ...
                            >
                                ...
                                <option value="non-client" className="font-medium text-sky-400">Solicitante Externo</option>
                            </select>
                            {isNonClient && (
                ...
                {isEditing ? (
                    <div className="grid grid-cols-2 gap-4 pt-2">
                        <div className="relative">
                            <label className="text-sm font-medium text-gray-500">Valor Tarea</label>
                        ...
                        <div className="relative">
                            <label className="text-sm font-medium text-gray-500">Abono</label>
                            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 mt-3">$</span>
                    ...
                    (editedTask.cost !== undefined && editedTask.cost > 0) && (
                        <div className="grid grid-cols-3 gap-4 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg mt-2">
                            ...
                            <div>
                                ...
                                <p className="font-medium text-gold">${(editedTask.cost - (editedTask.advancePayment || 0)).toFixed(2)}</p>
                            </div>
                        </div>
                    )
                )}
                ...
                <div>
                    ...
                    {isEditing ? (
                        <input type="date" value={safeFormat(editedTask.dueDate, 'yyyy-MM-dd')} onChange={e => setEditedTask({ ...editedTask, dueDate: new Date(e.target.value).toISOString() })} className="w-full p-2 mt-1 bg-gray-100 dark:bg-gray-800 rounded" />
                    ) : (
                        <p className="dark:text-white">{safeFormat(editedTask.dueDate, 'eeee, d MMMM yyyy')}</p>
                    )}
                </div>
                <div>
                ...
                {(task as any).isAutoGenerated && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl mt-4">
                        <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest block mb-2">Paso Siguiente</label>
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-200">
                            Subir el comprobante PDF de la declaración para liquidar esta orden automáticamente.
                        </p>
                    </div>
                )}
                ...
                {isEditing && <button onClick={handleSave} className="w-full mt-4 p-3 bg-gold text-black font-medium rounded-lg hover:bg-gold-dark transition-colors">Guardar Tarea</button>}
            </div>
        </div>
    );
};
