...
const App: React.FC = () => {
  // Navigation State
  const [appState, setAppState] = useState<'landing' | 'login' | 'dashboard' | 'services'>(() => {
      if (window.location.pathname === '/services') return 'services';
      return 'landing';
  });

  // Public User State (For Landing/Services)
  ...
  useEffect(() => {
    if (appState === 'dashboard' && isFirstLoad.current) {
  ...
  const handleLogoutConfirm = () => { setAppState('landing'); setShowLogoutConfirm(false); };

  const renderScreen = () => {
    switch (activeScreen) {
      case 'home':
        // Updated to pass clients and tasks to HomeScreen for Command Center Dashboard
  ...
  if (appState === 'login') return <LoginScreen onSuccess={() => { setAppState('dashboard'); setShowSplash(true); }} onBack={() => setAppState('landing')} />;
