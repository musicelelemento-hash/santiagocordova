...
const OBLIGATION_GROUPS = [
    ...
    { id: 'recent', label: 'Recientes', icon: Sparkle, color: 'text-amber-600 bg-amber-50 dark:text-amber-400 dark:bg-amber-900/20' }, 
...
interface ClientsScreenProps {
  ...
  navigate: (screen: Screen, options?: { 
    clientFilter?: ClientFilter, 
    taskFilter?: { clientId?: string; taskId?: string }, 
    initialClientData?: Partial<Client>, 
    initialTaskData?: Partial<Task>, 
    clientIdToView?: string 
  }) => void;
...
export const ClientsScreen: React.FC<ClientsScreenProps> = ({
  ...
  sriCredentials
    ...
    const [sortOption, setSortOption] = useState<'9th_digit' | 'name' | 'recent'>('9th_digit');
    ...
    }, [clients, searchTerm, filterOption, activeGroupTab, regimeFilter]);
    
    const sortedClients = useMemo(() => {
        return [...filteredClients].sort((a, b) => {
            if (sortOption === 'recent' || activeGroupTab === 'recent') {
    ...
    }, [filteredClients, sortOption, activeGroupTab]);
        ...
        <div>
                    ...
                    {isSortMenuOpen && (
                        <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl z-10 border border-slate-100 dark:border-gray-700 p-2 animate-fade-in-down">
                            ...
                            <div className="flex flex-col space-y-1">
                                <button onClick={() => setSortOption('9th_digit')} className={`text-left px-3 py-2 text-xs font-medium rounded-lg transition-colors ${sortOption === '9th_digit' ? 'bg-sky-50 text-sky-700 dark:bg-sky-900/30' : 'hover:bg-slate-50 dark:hover:bg-gray-700'}`}>Vencimiento SRI</button>
                                <button onClick={() => setSortOption('recent')} className={`text-left px-3 py-2 text-xs font-medium rounded-lg transition-colors ${sortOption === 'recent' ? 'bg-sky-50 text-sky-700 dark:bg-sky-900/30' : 'hover:bg-slate-50 dark:hover:bg-gray-700'}`}>Agregados Recientemente</button>
            ...
            {activeGroupTab === 'devolucion' && (
                <div className="mb-6 p-6 bg-gradient-to-r from-emerald-500 to-teal-600 rounded-2xl text-white shadow-lg animate-fade-in-down flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div>
                        ...
                        <p className="text-emerald-100 text-sm max-w-md">Incluye: Declaración de Renta + Anexo Gastos Personales + Solicitud de Devolución de Retenciones.</p>
                    </div>
                    <button onClick={() => setIsComboModalOpen(true)} className="px-6 py-3 bg-white text-emerald-700 font-black rounded-xl shadow-lg hover:bg-emerald-50 transition-transform transform hover:scale-105 flex items-center gap-2"><Plus size={20} strokeWidth={3}/> Vender Combo $25.00</button>
                </div>
            )}
            
            <div className="h-[calc(100vh-320px)] min-h-[500px] w-full">
                 {sortedClients.length > 0 ? (
                    <VirtualClientList clients={sortedClients} serviceFees={serviceFees} onView={handleOpenClientDetails} onQuickAction={handleQuickAction}/>
                 ) : (
                    <div className="col-span-full py-20 text-center">
                        <div className="inline-flex p-4 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 mb-4"><Search size={32}/></div>
                        <p className="text-slate-500 font-medium">No se encontraron clientes con este filtro.</p>
                    </div>
                 )}
            </div>

            {/* MODAL DE CREACIÓN/EDICIÓN - Con bloqueo de cierre accidental */}
            <Modal 
                isOpen={isModalOpen} 
                onClose={() => setIsModalOpen(false)} 
                title="Agregar Nuevo Cliente"
                disableBackdropClick={true} // Bloqueo activado
            >
                 <ClientForm 
            ...
            <Modal isOpen={isComboModalOpen} onClose={() => setIsComboModalOpen(false)} title="Vender Combo Devolución">
                <div className="space-y-4">
                    <p className="text-slate-600 dark:text-slate-300 text-sm">Seleccione un cliente existente para generar la tarea del Combo ($25.00) o cree uno nuevo.</p>
                    <div className="grid gap-2 max-h-[300px] overflow-y-auto">
                        {clients.filter(c => c.isActive).map(c => (
                            <button key={c.id} onClick={() => handleAddComboTask(c.id)} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 border border-slate-200 dark:border-slate-700 rounded-xl transition-colors group text-left">
                                <div>
                    ...
                    <div className="border-t border-slate-200 pt-4 mt-2">
                        <button onClick={() => { setIsComboModalOpen(false); setIsModalOpen(true); }} className="w-full py-3 bg-brand-navy text-white font-bold rounded-xl flex items-center justify-center gap-2"><Plus size={18}/> Crear Nuevo Cliente con Tarifa Pro</button>
                    </div>
                </div>
            </Modal>
        </div>
