...
import { Trash2, UploadCloud, Download, Info, Edit, Save, XCircle, CheckCircle, Loader, Printer, Clipboard, DollarSign, AlertTriangle, ChevronUp, ChevronDown, Palette, ChevronRight, MessageSquare, ToggleLeft, ToggleRight, CalendarClock, Key, Cloud, Database, Link, FileText, HardDrive, RefreshCcw } from 'lucide-react';
import { exportClientsToCSV, parseClientsFromCSV, parseBrowserPasswordsCSV, parseCredentialsCSV } from '../services/csv';
import { addAdvancePayments, getClientServiceFee, createClientFromExtraction } from '../services/clientService';
import { extractDataFromSriPdf } from '../services/pdfExtraction';
import { generateBackupFile, parseBackupFile, BackupData } from '../services/backupService';
import { Modal } from '../components/Modal';
import { format, isPast } from 'date-fns';
import { es } from 'date-fns/locale';
import { getDueDateForPeriod, formatPeriodForDisplay, getIdentifierSortKey, validateRuc } from '../services/sri';
...
interface SettingsScreenProps {
  ...
  setSriCredentials?: React.Dispatch<React.SetStateAction<Record<string, string>>>;
  webOrders: WebOrder[];
  setWebOrders: React.Dispatch<React.SetStateAction<WebOrder[]>>;
...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ clients, setClients, tasks, setTasks, serviceFees, setServiceFees, reminderConfig, setReminderConfig, navigate, setSriCredentials, webOrders, setWebOrders, sriCredentials }) => {
  ...
  const handleFullBackupExport = () => {
      try {
          ...
          alert('✅ Archivo de respaldo generado correctamente.');
      } catch (e: any) {
  ...
  const handleFullBackupImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!window.confirm("⚠️ IMPORTANTE: Esta acción reemplazará los datos actuales con los del archivo de respaldo. ¿Está seguro?")) {
      ...
      try {
          ...
          if(data.serviceFees) setServiceFees(data.serviceFees);
          if(data.reminderConfig) setReminderConfig(data.reminderConfig);
          if(data.webOrders) setWebOrders(data.webOrders);
          if(data.sriCredentials && setSriCredentials) setSriCredentials(data.sriCredentials);

          alert(`✅ Restauración exitosa.\n${data.clients?.length || 0} clientes recuperados.`);
      } catch (e: any) {
  ...
  const handleBatchPdfClick = () => {
      pdfBatchInputRef.current?.click();
  };
  ...
  const handleBatchPdfUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (let i = 0; i < fileArray.length; i++) {
            ...
            try {
                 ...
                 if (file.size > 5 * 1024 * 1024) {
                     throw new Error("Archivo muy pesado (>5MB)");
                 }

                 const extracted = await extractDataFromSriPdf(file);
                 const newClient = createClientFromExtraction(extracted, sriCredentials);
            ...
            setBatchProgress(Math.round(((i + 1) / fileArray.length) * 100));
        }

        if (results.length > 0) {
            setClients(prev => {
                // Update existing or add new, indexed by RUC
                const clientMap = new Map<string, Client>(prev.map(c => [c.ruc, c]));
                let updatedCount = 0;
                let createdCount = 0;

                results.forEach(c => {
                    if (clientMap.has(c.ruc)) {
                        const existing = clientMap.get(c.ruc)!;
                        clientMap.set(c.ruc, { ...existing, ...c, id: existing.id });
                        updatedCount++;
                    } else {
                        clientMap.set(c.ruc, c);
                        createdCount++;
                    }
                });
        ...
        if (errors.length > 0) {
            alert(`⚠️ Errores en ${errors.length} archivos:\n${errors.slice(0, 5).join('\n')}${errors.length > 5 ? '...' : ''}`);
        }
  ...
  return (
    <div className="relative">
      ...
      <div className="space-y-6">
        ...
        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md border border-slate-200 dark:border-slate-700">
            <h3 className="font-bold text-xl dark:text-white mb-2 flex items-center gap-2">
                <Cloud className="text-sky-500" size={24}/> Conexión a la Nube (Google Sheets)
            </h3>
            <p className="text-gray-600 dark:text-gray-400 mb-4 text-sm">
                Si descargó el proyecto de GitHub, es probable que necesite restablecer la conexión con su base de datos.
                Ingrese aquí la URL de su Script de Google Apps (Web App URL).
            </p>
            <div className="flex gap-2">
                <div className="relative flex-grow">
                    ...
                    <input 
                        ...
                        className="w-full pl-9 p-2 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded text-xs font-mono"
                    />
                </div>
                <button 
                    onClick={handleSaveBackendUrl}
                    disabled={isSavingUrl}
                    className="px-4 py-2 bg-sky-600 text-white font-bold rounded hover:bg-sky-700 transition-colors text-xs flex items-center gap-2"
                >
                    {isSavingUrl ? <Loader className="animate-spin" size={14}/> : <Database size={14}/>}
                    Guardar
                </button>
            </div>
        ...
        <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-md">
            <h3 className="font-bold text-xl mb-4 dark:text-white">Gestión de Datos y Guías</h3>
             <div className="space-y-4">
                <CollapsibleGuide title="Leyenda de Estados y Colores" icon={Palette}>
                 ...
                 {/* NUEVO PANEL: Backup & Restauración */}
                 <CollapsibleGuide title="Copia de Seguridad y Migración" icon={HardDrive}>
                    <p className="text-sm mb-4">
                        Descargue una copia completa de su base de datos (Clientes, Tareas, Configuración, Credenciales) para guardarla en un lugar seguro o transferirla a otro dispositivo.
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800">
                            ...
                            <p className="text-xs text-emerald-600 dark:text-emerald-400 mb-3">
                                Genera un archivo <code>.json</code> con toda la información actual del sistema.
                            </p>
                            <button 
                                onClick={handleFullBackupExport}
                                className="w-full py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-xs transition-colors shadow-sm"
                            >
                        ...
                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                            <h4 className="font-bold text-blue-800 dark:text-blue-300 mb-2 flex items-center gap-2">
                                <RefreshCcw size={16}/> Restaurar Datos
                            </h4>
                            <p className="text-xs text-blue-600 dark:text-blue-400 mb-3">
                                Carga un archivo de respaldo previo. <strong>⚠️ Esto reemplazará los datos actuales.</strong>
                            </p>
                            <input 
                                type="file" 
                                ref={backupInputRef}
                            ...
                            <button 
                                ...
                                className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg text-xs transition-colors shadow-sm flex items-center justify-center gap-2"
                            >
                ...
                <CollapsibleGuide title="Base de Datos de Claves SRI" icon={Key}>
                    <p className="mb-2">
                        Importe un archivo CSV de contraseñas exportado desde su navegador (Chrome, Edge, etc.). 
                        El sistema buscará automáticamente las credenciales relacionadas con <code>sri.gob.ec</code>.
                    </p>
                    <ul className="list-disc list-inside space-y-1 mb-4 text-xs">
                        <li>Esta opción <strong>NO crea clientes</strong>, solo alimenta la base de datos interna para autocompletar.</li>
                        <li>Si el cliente ya existe, se actualiza su clave.</li>
                    </ul>
                    <div className="flex items-center justify-between p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                        <div>
                            <p className="font-bold text-blue-800 dark:text-blue-300 text-sm">Cargar CSV de Navegador</p>
                            <p className="text-xs text-blue-600 dark:text-blue-400">Alimenta el autocompletado de claves</p>
                        </div>
                        <input
                            type="file"
                            ...
                            className="hidden"
                        />
                        <button onClick={handlePasswordImportClick} className="px-4 py-2 bg-blue-600 text-white text-xs font-bold rounded-md hover:bg-blue-700 transition-colors shadow-sm">
                            Importar Claves
                        </button>
                    </div>
                </CollapsibleGuide>
            ...
            <div className="flex flex-wrap gap-4 mt-6">
                ...
                <button onClick={handleExport} className="flex-1 flex items-center justify-center space-x-2 p-3 bg-gold text-black font-bold rounded-lg hover:bg-gold-dark transition-colors">
                 ...
                 {/* Nuevo Botón: Importar Lote PDFs */}
                <input 
                    type="file" 
                ...
                <button 
                    ...
                    className="flex-1 flex items-center justify-center space-x-2 p-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 relative overflow-hidden"
                >
                    {isBatchProcessing ? (
                         <>
                            <div className="absolute inset-0 bg-blue-500" style={{ width: `${batchProgress}%` }}></div>
                            <span className="relative z-10 flex items-center gap-2"><Loader size={16} className="animate-spin"/> {batchProgress}%</span>
                         </>
                    ) : (
                         <>
                            <FileText size={20} />
                            <span>Importar Lote PDFs</span>
                         </>
                    )}
                </button>
            </div>
